...
function applyTheme(t){document.documentElement.dataset.theme=t;
  if(metaTheme)metaTheme.content=t==='dark'?'#191b21':'#e9eef7';
...
var PALETTES={
 ruri:{name:'瑠璃×朱',light:{bg:'#e9eef7',card:'#f6f8fc',blue:'#174a99',hover:'#b23a12',like:'#b23a12',sel:'rgba(23,74,153,.15)'},dark:{bg:'#191b21',card:'#22242b',blue:'#8fb0de',hover:'#e08856',like:'#e08856',sel:'rgba(143,176,222,.2)'}},
 kari:{name:'蒸栗×墨',light:{bg:'#edecd6',card:'#f7f6e7',blue:'#43511b',hover:'#2e2a20',like:'#43511b',sel:'rgba(67,81,27,.16)'},dark:{bg:'#1d1f17',card:'#262a1e',blue:'#c6ce96',hover:'#e0e4ae',like:'#c6ce96',sel:'rgba(198,206,150,.2)'}},
 nadeshiko:{name:'撫子×苏芳',light:{bg:'#f5e5ea',card:'#fcf4f6',blue:'#7a2f42',hover:'#b23a52',like:'#b23a52',sel:'rgba(122,47,66,.14)'},dark:{bg:'#211b1e',card:'#2a2226',blue:'#e3a8b5',hover:'#d98c9f',like:'#d98c9f',sel:'rgba(227,168,181,.18)'}},
 asagi:{name:'浅葱×紺青',light:{bg:'#e1edee',card:'#f2f8f8',blue:'#0f5a5e',hover:'#0a3d40',like:'#0f5a5e',sel:'rgba(15,90,94,.14)'},dark:{bg:'#171c1e',card:'#1f2628',blue:'#7ec7d8',hover:'#a5dce5',like:'#7ec7d8',sel:'rgba(126,199,216,.18)'}}};
var curPalette=(function(){try{return localStorage.getItem('sx-palette')||'ruri'}catch(e){return 'ruri'}})();
function applyPalette(pid){var pal=PALETTES[pid]||PALETTES.ruri;curPalette=pid;
 var m=document.documentElement.dataset.theme==='dark'?pal.dark:pal.light;
 var st=document.documentElement.style;
 st.setProperty('--bg',m.bg);st.setProperty('--card',m.card);
 st.setProperty('--blue',m.blue);st.setProperty('--hover',m.hover);
 st.setProperty('--like',m.like);st.setProperty('--sel',m.sel);
 document.querySelectorAll('.pal-dot').forEach(function(d){d.classList.toggle('on',d.dataset.pal===pid)});
...
def toggles_html():
    dots = "".join(
        f'<button class="pal-dot" data-pal="{pid}" style="--c:{c}" type="button" title="{name}" aria-label="{name}"></button>'
        for pid, c, name in [('ruri', '#195fae', '瑠璃×朱'), ('kari', '#8a9a3b', '蒸栗×墨'),
                              ('nadeshiko', '#c4657e', '撫子×苏芳'), ('asagi', '#2e8a8e', '浅葱×紺青')])
...
CSS = """
:root{--bg:#e9eef7;--ink:#201f1c;--body:#45433c;--muted:#7c786c;--border:#dde3ef;--blue:#174a99;--hover:#b23a12;--like:#b23a12;--card:#f6f8fc;--sel:rgba(23,74,153,.15);color-scheme:light;
...
.pal-dots{display:flex;gap:7px;align-items:center;margin-right:6px}
.pal-dot{width:17px;height:17px;border-radius:50%;border:none;background:var(--c);cursor:pointer;box-shadow:inset 0 0 0 1px rgba(0,0,0,.15);transition:transform .15s;padding:0}
.pal-dot:hover{transform:scale(1.2)}
.pal-dot.on{outline:2px solid var(--ink);outline-offset:2px}
...
<title>{title}</title>
<meta name="theme-color" content="#e9eef7" id="metaTheme">
{FAVICON}
{og}{can}
{THEME_HEAD}
