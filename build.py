...
function applyTheme(t){document.documentElement.dataset.theme=t;document.documentElement.style.colorScheme=t;
  themeBtn.setAttribute('aria-pressed',t==='dark'?'true':'false');
  if(typeof applyPalette==='function')applyPalette(curPalette);}
...
var PALETTES={
 tekkon:{name:'铁绀 × 天青',light:{bg:'#fcfbf8',card:'#ffffff',ink:'#132a30',body:'#354d54',muted:'#5f707a',border:'#d6e3e7',blue:'#005d78',hover:'#003443',like:'#a3422d',sel:'rgba(25,95,174,.17)',accent:'#003443',accentInk:'#fff',shadow:'0 10px 30px rgba(0,52,67,.11)'},dark:{bg:'#07171c',card:'#0c2229',ink:'#eaeff3',body:'#bed1d7',muted:'#7f9aa2',border:'#183943',blue:'#97cfea',hover:'#d8eff7',like:'#f09a82',sel:'rgba(151,207,234,.24)',accent:'#97cfea',accentInk:'#07171c',shadow:'0 12px 34px rgba(0,0,0,.3)'}},
 tokiwa:{name:'常盘 × 秘色',light:{bg:'#fafcf8',card:'#ffffff',ink:'#17311f',body:'#395844',muted:'#617164',border:'#d8e5d7',blue:'#01713f',hover:'#00542e',like:'#9b4638',sel:'rgba(1,113,63,.17)',accent:'#01713f',accentInk:'#fff',shadow:'0 10px 30px rgba(1,113,63,.1)'},dark:{bg:'#0a1911',card:'#10231a',ink:'#edf5ef',body:'#bfd5c6',muted:'#859b8a',border:'#233b2a',blue:'#8fcda7',hover:'#b9ddb7',like:'#e09a8b',sel:'rgba(143,205,167,.22)',accent:'#5fa97b',accentInk:'#07150d',shadow:'0 12px 34px rgba(0,0,0,.3)'}},
 edomurasaki:{name:'江户紫 × 紫苑',light:{bg:'#fbf9fc',card:'#ffffff',ink:'#2d2032',body:'#53465a',muted:'#756878',border:'#e5dde8',blue:'#6d1d74',hover:'#581660',like:'#9f2f6c',sel:'rgba(88,22,96,.16)',accent:'#581660',accentInk:'#fff',shadow:'0 10px 30px rgba(88,22,96,.1)'},dark:{bg:'#160f18',card:'#201522',ink:'#f3edf5',body:'#d5c6d9',muted:'#9e8fa3',border:'#36233a',blue:'#bba8cf',hover:'#ddd0e8',like:'#e390b8',sel:'rgba(187,168,207,.23)',accent:'#8b68a1',accentInk:'#160f18',shadow:'0 12px 34px rgba(0,0,0,.31)'}},
 ouni:{name:'黄丹 × 团栗',light:{bg:'#faf1e6',card:'#fffaf4',ink:'#3f312c',body:'#5c463c',muted:'#7c6255',border:'#e8d3c2',blue:'#b33e20',hover:'#7f2b18',like:'#9b2b2b',sel:'rgba(227,92,47,.18)',accent:'#e35c2f',accentInk:'#fff',shadow:'0 10px 30px rgba(121,74,46,.12)'},dark:{bg:'#1b120e',card:'#251915',ink:'#f7e8dc',body:'#d7c0b2',muted:'#a58b7d',border:'#3e2b23',blue:'#ff9a72',hover:'#ffc2aa',like:'#ff8a78',sel:'rgba(255,154,114,.22)',accent:'#e35c2f',accentInk:'#1b120e',shadow:'0 12px 34px rgba(0,0,0,.32)'}}};
var curPalette=(function(){try{var p=localStorage.getItem('sx-palette');return PALETTES[p]?p:'tekkon'}catch(e){return 'tekkon'}})();
function applyPalette(pid){var pal=PALETTES[pid]||PALETTES.tekkon;curPalette=pid;
 var m=document.documentElement.dataset.theme==='dark'?pal.dark:pal.light;
 var st=document.documentElement.style;
 st.setProperty('--bg',m.bg);st.setProperty('--card',m.card);st.setProperty('--surface',m.card);
 st.setProperty('--ink',m.ink);st.setProperty('--body',m.body);st.setProperty('--muted',m.muted);st.setProperty('--border',m.border);
 st.setProperty('--blue',m.blue);st.setProperty('--hover',m.hover);st.setProperty('--like',m.like);st.setProperty('--sel',m.sel);
 st.setProperty('--accent-fill',m.accent);st.setProperty('--accent-ink',m.accentInk);st.setProperty('--dock-shadow',m.shadow);
 if(metaTheme)metaTheme.content=m.bg;
 document.querySelectorAll('.pal-dot').forEach(function(d){var on=d.dataset.pal===pid;d.classList.toggle('on',on);d.setAttribute('aria-pressed',on?'true':'false')});
 try{localStorage.setItem('sx-palette',pid);}catch(e){}}
document.querySelectorAll('.pal-dot').forEach(function(d){
 d.addEventListener('click',function(){applyPalette(d.dataset.pal);});});
...
def toggles_html():
    dots = "".join(
        f'<button class="pal-dot" data-pal="{pid}" style="--c1:{c1};--c2:{c2}" type="button" title="{name}" aria-label="{name}"></button>'
        for pid, c1, c2, name in [('tekkon', '#003443', '#97cfea', '铁绀 × 天青'),
                                  ('tokiwa', '#01713f', '#b9ddb7', '常盘 × 秘色'),
                                  ('edomurasaki', '#581660', '#a28ebf', '江户紫 × 紫苑'),
                                  ('ouni', '#e35c2f', '#794a2e', '黄丹 × 团栗')])
    return ('<div class="toggles" aria-label="显示设置">'
            '<div class="pal-dots" role="group" aria-label="配色方案">' + dots + '</div>'
            '<span class="toggle-divider" aria-hidden="true"></span>'
            '<button id="themeBtn" class="mode-toggle" type="button" aria-label="Toggle theme" aria-pressed="false">'
            '<svg class="sun" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><circle cx="12" cy="12" r="3.5"/><path d="M12 2v2.2M12 19.8V22M4.93 4.93l1.56 1.56M17.51 17.51l1.56 1.56M2 12h2.2M19.8 12H22M4.93 19.07l1.56-1.56M17.51 6.49l1.56-1.56"/></svg>'
            '<svg class="moon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M20 15.1A8.2 8.2 0 0 1 8.9 4a8.2 8.2 0 1 0 11.1 11.1Z"/></svg>'
            '<span class="mode-knob" aria-hidden="true"></span></button>'
...
CSS = """
:root{--bg:#fcfbf8;--ink:#132a30;--body:#354d54;--muted:#5f707a;--border:#d6e3e7;--blue:#005d78;--hover:#003443;--like:#a3422d;--card:#fff;--surface:#fff;--sel:rgba(25,95,174,.17);--accent-fill:#003443;--accent-ink:#fff;--dock-shadow:0 10px 30px rgba(0,52,67,.11);color-scheme:light;
...
[data-theme="dark"]{--bg:#07171c;--ink:#eaeff3;--body:#bed1d7;--muted:#7f9aa2;--border:#183943;--blue:#97cfea;--hover:#d8eff7;--like:#f09a82;--card:#0c2229;--surface:#0c2229;--sel:rgba(151,207,234,.24);--accent-fill:#97cfea;--accent-ink:#07171c;--dock-shadow:0 12px 34px rgba(0,0,0,.3);color-scheme:dark;}
@media(prefers-color-scheme:dark){:root:not([data-theme]){--bg:#07171c;--ink:#eaeff3;--body:#bed1d7;--muted:#7f9aa2;--border:#183943;--blue:#97cfea;--hover:#d8eff7;--like:#f09a82;--card:#0c2229;--surface:#0c2229;--sel:rgba(151,207,234,.24);color-scheme:dark;}}
...
.toggles{position:fixed;top:14px;right:16px;z-index:50;display:flex;align-items:center;gap:8px;padding:6px;background:color-mix(in srgb,var(--card) 88%,transparent);border:1px solid var(--border);border-radius:999px;box-shadow:var(--dock-shadow);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);transition:background-color .25s,border-color .25s,box-shadow .25s}
.tgl{width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:var(--mono);font-size:12px;font-weight:600;color:var(--muted);background:transparent;border:0;cursor:pointer;transition:color .15s,background-color .15s,transform .15s}
.tgl:hover{color:var(--blue);background:var(--bg);transform:scale(1.04)}
.pal-dots{display:flex;gap:5px;align-items:center;padding:0 4px}
.pal-dot{position:relative;width:31px;height:24px;border-radius:8px;border:0;background:linear-gradient(135deg,var(--c1) 0 52%,var(--c2) 52% 100%);cursor:pointer;box-shadow:inset 0 0 0 1px rgba(0,0,0,.12);transition:transform .15s,box-shadow .15s;padding:0}
.pal-dot:hover{transform:translateY(-1px) scale(1.06)}
.pal-dot.on{box-shadow:inset 0 0 0 2px var(--card),0 0 0 2px var(--ink)}
.mode-toggle{position:relative;width:58px;height:32px;padding:0;flex:none;border:0;border-radius:999px;background:var(--bg);color:var(--muted);cursor:pointer;box-shadow:inset 0 0 0 1px var(--border);transition:background-color .25s,box-shadow .25s}
.mode-toggle svg{position:absolute;top:9px;width:14px;height:14px;z-index:1;pointer-events:none}.mode-toggle .sun{left:9px}.mode-toggle .moon{right:9px}
.mode-toggle .sun{color:var(--accent-ink)}[data-theme="dark"] .mode-toggle .sun{color:var(--muted)}[data-theme="dark"] .mode-toggle .moon{color:var(--accent-ink)}
.mode-knob{position:absolute;left:3px;top:3px;width:26px;height:26px;border-radius:50%;background:var(--accent-fill);box-shadow:0 2px 7px rgba(0,0,0,.18);transition:transform .25s cubic-bezier(.2,.8,.2,1),background-color .25s}
[data-theme="dark"] .mode-knob{transform:translateX(26px)}.mode-toggle:hover{box-shadow:inset 0 0 0 1px var(--blue)}
.toggle-divider{width:1px;height:20px;background:var(--border)}
...
.tagchip.on{background:var(--accent-fill);border-color:var(--accent-fill);color:var(--accent-ink)}
...
@media(max-width:640px){.page{padding:60px 20px 56px}.toggles{top:10px;right:10px;gap:5px;padding:5px}.pal-dots{gap:4px;padding:0 3px}.pal-dot{width:27px;height:22px}article h1{font-size:24px}h1.page-title{font-size:28px}.plist a{gap:12px}}
...
FAVICON = '<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 32 32\'%3E%3Crect width=\'32\' height=\'32\' rx=\'7\' fill=\'%23fcfbf8\'/%3E%3Crect x=\'4\' y=\'4\' width=\'11\' height=\'11\' fill=\'%23003443\'/%3E%3Crect x=\'17\' y=\'17\' width=\'11\' height=\'11\' fill=\'%23003443\'/%3E%3Crect x=\'17\' y=\'4\' width=\'11\' height=\'11\' fill=\'none\' stroke=\'%2397cfea\' stroke-width=\'2\'/%3E%3Crect x=\'4\' y=\'17\' width=\'11\' height=\'11\' fill=\'none\' stroke=\'%2397cfea\' stroke-width=\'2\'/%3E%3C/svg%3E">'
...
<title>{title}</title>
<meta name="theme-color" content="#fcfbf8" id="metaTheme">
{FAVICON}
{og}{can}
{THEME_HEAD}
