...
$$\theta_0 \approx \pm\sqrt{\frac{6\pi^2}{24 + \pi^2}\cdot\frac{T_c - T}{T_c}}$$
...
>
> $$\left(\frac{\sin\theta}{\theta}\right)'=\frac{\theta\cos\theta-\sin\theta}{\theta^2}<0\qquad(0<\theta<\pi/2),$$
>
> 因为 $\sin\theta-\theta\cos\theta$ 在原点为零，导数为 $\theta\sin\theta>0$。这样就不用对整个乘积硬求导了。
...
$$\omega = \sqrt{\frac{g}{r}\left(\frac{8nRT}{\pi^2 m g r} - 1\right)} = \sqrt{\frac{g}{r}\cdot\frac{T - T_c}{T_c}}$$

温度略低于 $T_c$ 时（$\theta_0 \ll 1$，代入 (3) 的近似）：

$$\omega_0 \approx \sqrt{\frac{2g}{r}\cdot\frac{T_c - T}{T_c}}$$
...
$$\frac12 mr^2\dot\theta^2+U_{\mathrm{eff}}(\theta)=E.$$
...
![题解图 4. C、D 两杆的受力图](../assets/blog/cpho40/fig-4-solution.svg)
